 ...
 *   "North America": {
 *     "USA": [
 *       { id: "dfw", city: "Dallas", country: "USA", continent: "North America" },
 *       { id: "lax", city: "Los Angeles", country: "USA", continent: "North America" }
 *     ],
 *     "Canada": [
 *       { id: "yyz", city: "Toronto", country: "Canada", continent: "North America" }
 *     ]
 *   },
 *   "Europe": {
 *     "Netherlands": [
 *       { id: "ams", city: "Amsterdam", country: "Netherlands", continent: "Europe" }
 *     ]
 *   }
 * }
 */
  ...
  try {
    ...
    if (process.env.VULTR_FIREWALL_ENABLED === 'true') {
      try {
        ...
        const isActive = await checkInstanceStatus(response.instance.id, 'active', 'running', 30);
  ...
  async execute(interaction) {
    ...
    try {
      ...
      return interaction.editReply(
        ...
        `📊 You can check the status anytime with \`/status\`\n` +
