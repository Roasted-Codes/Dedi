...
const { 
  ...
  ButtonBuilder,           // Helper to build buttons for Discord interactions.
  ButtonStyle             // Helper to define button styles for Discord interactions.
 ...
 * @param {string} region - The region to deploy the new server in.
 * @returns {Promise<Object>} A promise that resolves with the details of the newly created instance.
 * @throws Will throw an error if the API call fails.
 */
async function createInstanceFromSnapshot(snapshotId, label, region = null) {
  try {
    const regionToUse = region || process.env.VULTR_REGION || "dfw";
    const planToUse = process.env.VULTR_PLAN || "vc2-1c-1gb";
...
// Fetch available regions from Vultr
async function getRegions() {
  try {
    const response = await vultr.regions.listRegions();
    console.log('Vultr regions API response:', JSON.stringify(response, null, 2));
...
const CONTINENT_MAP = {
  "North America": ["dfw", "ewr", "lax", "mia", "ord", "sea", "sjc", "yto", "atl", "hnl", "mex", "chi"],
  "Europe": ["ams", "cdg", "fra", "lhr", "mad", "man", "sto", "waw"],
  "Asia": ["blr", "bom", "del", "icn", "itm", "nrt", "sgp", "tlv"],
  "Australia": ["mel", "syd"],
  "South America": ["sao", "scl"],
  "Africa": ["jnb"],
};

async function getContinents(regions) {
  // Group regions by continent
  const continents = {};
  for (const region of regions) {
    if (!continents[region.continent]) continents[region.continent] = [];
...
const createCommand = {
  ...
  async execute(interaction) {
    ...
    const serverName = interaction.options.getString('name') || `${interaction.user.username}'s Server`;
    await interaction.editReply({
      content: `Where do you want to deploy your server?`,
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`quick_create_dfw|${serverName}`)
            .setLabel('Quick Create (Dallas, US)')
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId(`choose_by_continent|${serverName}`)
            .setLabel('Choose by Continent')
...
// Consolidated select menu handler for continent and region selection
client.on('interactionCreate', async interaction => {
  if (!interaction.isStringSelectMenu()) return;
  ...
  if (interaction.customId === 'select_continent') {
    await interaction.deferUpdate();
    const [continent, serverName] = interaction.values[0].split('|');
    const regions = await getRegions();
    const filtered = regions.filter(r => r.continent === continent);

    // Build region dropdown (up to 25)
    const options = filtered.slice(0, 25).map(region => ({
      label: `${region.city} (${region.id})`,
      description: region.country,
      value: `${region.id}|${serverName}`
    }));
    ...
    await interaction.editReply({
      content: `Choose a region in ${continent}:`,
      components: [row]
    });
    return;
  }
  ...
  if (interaction.customId === 'select_region') {
    await interaction.deferUpdate();
    const [region, serverName] = interaction.values[0].split('|');
    const snapshots = await getSnapshots();
    const snapshotId = process.env.VULTR_SNAPSHOT_ID || snapshots[0].id;
    await interaction.editReply({
      content: `🔄 Creating server in ${region}...`,
      components: [] // Remove dropdown immediately
    });
    const instance = await createInstanceFromSnapshot(snapshotId, serverName, region);
    if (!instance || !instance.id) {
      return interaction.editReply({
        content: '❌ Failed to create the server. Please try again later.',
        components: []
      });
    }
    ...
    if (readyInstance) {
      return interaction.editReply({
        content: `🎉 <@${interaction.user.id}>, your server "${serverName}" is ready!\nIP Address: \`${readyInstance.main_ip}\``,
        components: []
      });
    } else {
      return interaction.editReply({
        content: `⚠️ <@${interaction.user.id}>, server "${serverName}" started but taking longer than usual.\nUse \`/status\` to check when it's ready.`,
        components: []
      });
    }
  }
...
client.on('interactionCreate', async interaction => {
  ...
  const [action, serverName] = interaction.customId.split('|');

  if (action === 'quick_create_dfw') {
    await interaction.deferUpdate();
    // Remove the buttons immediately
    await interaction.editReply({
      content: '🔄 Creating server in Dallas (dfw)...',
      components: []
    });
    // Use your snapshot logic as before
    const snapshots = await getSnapshots();
    const snapshotId = process.env.VULTR_SNAPSHOT_ID || snapshots[0].id;
    const instance = await createInstanceFromSnapshot(snapshotId, serverName, 'dfw');
    // ...rest of your instance creation logic...
    return;
  }

  if (action === 'choose_by_continent') {
    await interaction.deferUpdate();
    const regions = await getRegions();
    const continents = await getContinents(regions);

    // Build continent dropdown
    const continentOptions = Object.keys(continents).map(continent => ({
      label: continent,
      value: `${continent}|${serverName}`
    }));

    const row = new ActionRowBuilder().addComponents(
      new StringSelectMenuBuilder()
        .setCustomId('select_continent')
        .setPlaceholder('Select a continent')
        .addOptions(continentOptions)
    );

    await interaction.editReply({
      content: 'Choose a continent:',
      components: [row]
    });
    return;
  }
